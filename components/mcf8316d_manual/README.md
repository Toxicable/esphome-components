...
Brake and direction writes now log immediate register readback (`PIN_CONFIG` / `PERI_CONFIG1`), and commanded-run diagnostics log `[loop_control] CTRL diag` with decoded `brake_sel`/`dir_sel` so you can verify the chip is not being held in brake by configuration.
