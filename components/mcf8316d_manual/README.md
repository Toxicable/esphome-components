...
For safety, the component forces speed to 0% on persistent faults, but now defers to configured lock protection modes (`retry_*`, `report_only`, `disabled`) for `LOCK_LIMIT`/`HW_LOCK_LIMIT` and motor-lock (`MTR_LCK`, `ABN_SPEED`, `ABN_BEMF`, `NO_MTR`) events so chip-managed retries are not preempted.
