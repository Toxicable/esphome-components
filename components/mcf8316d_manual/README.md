...
Optional `apply_hw_lock_report_only` button is a temporary diagnostic mode that sets `HW_LOCK_ILIMIT_MODE=report_only` (no protective shutdown on HW lock); use only for brief no-load debugging and then run `apply_startup_tune` to restore normal mode.
