...
MCF8316DDirectionSelect = mcf8316d_manual_ns.class_("MCF8316DDirectionSelect", select.Select)

CONFIG_SCHEMA = select.select_schema(
    MCF8316DDirectionSelect,
    options=["hardware", "cw", "ccw"],
...
async def to_code(config):
    var = await select.new_select(config, options=["hardware", "cw", "ccw"])
