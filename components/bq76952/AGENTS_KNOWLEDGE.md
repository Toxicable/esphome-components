...
- `cell_count` now also drives a boot-applied `Settings:Configuration:Vcell Mode (0x9304)` mask so only configured cells participate in cell-voltage protections; this avoids false CUV/COV on unused VC inputs for packs smaller than 16S.
