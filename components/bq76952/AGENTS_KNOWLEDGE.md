...
- Boot-applied `Settings:Configuration:Vcell Mode (0x9304)` now auto-detects active VC channels from live direct-cell readings; users can force a mask with `vcell_mode_mask` when a board needs explicit channel selection.
