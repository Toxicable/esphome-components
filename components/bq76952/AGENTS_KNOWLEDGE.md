...
- Boot-applied regulator, TS-pin, FET-option, protection, and boot-mode writes are always enabled; `apply_configuration` remains as a manual re-apply path rather than a substitute for boot application.
...
- Runtime controls/buttons (`output_enabled_control`, `autonomous_fet_control`, `clear_alarms`, `reset_passed_charge`, `apply_configuration`, `program_factory_otp`) emit explicit `Action:` / `Action result:` INFO logs.
