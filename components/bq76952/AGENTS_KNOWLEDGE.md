...
- Deferred or recovery-triggered configuration remains pending until every requested configuration group applies successfully. A failed Control Status read queues the shared configuration applicator immediately, so a BQ reset or I2C outage cannot recover into polling with reset-default TS/regulator/protection settings.
