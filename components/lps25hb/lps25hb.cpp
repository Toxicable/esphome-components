...
// CTRL_REG1 bits per datasheet: PD bit enables device; ODR=000 enables one-shot; BDU blocks partial
// updates. :contentReference[oaicite:1]{index=1}
static constexpr uint8_t CTRL1_PD = 0x80;
static constexpr uint8_t CTRL1_BDU = 0x04;
// ODR[2:0] are bits 6..4; leaving them 0 => one-shot mode enabled.
...
void LPS25HBComponent::setup() {
  ...
  if (who != WHO_AM_I_LPS25HB) {
    ESP_LOGE(
      TAG, "Unexpected WHO_AM_I (0x%02X). Expected 0x%02X (LPS25HB).", who, WHO_AM_I_LPS25HB
    );
    this->mark_failed();
    return;
  }
...
bool LPS25HBComponent::trigger_one_shot_() {
  // ONE_SHOT bit triggers a new dataset when ODR=000 (one-shot mode).
...
bool LPS25HBComponent::wait_data_ready_(uint32_t timeout_ms) {
  ...
  while (millis() - start < timeout_ms) {
    ...
    // STATUS_REG bits: P_DA and T_DA indicate new pressure/temp data; common ST pattern uses bits
    // 1:0.
    if ((status & 0x03) == 0x03) {
      return true;
    }
...
bool LPS25HBComponent::read_measurements_(float& temp_c, float& pressure_hpa) {
  ...
  int32_t p_raw =
    (static_cast<int32_t>(ph) << 16) | (static_cast<int32_t>(pl) << 8) | static_cast<int32_t>(pxl);
  // Sign-extend 24-bit if needed (datasheet describes pressure output as 24-bit).
  ...
  int16_t t_raw =
    static_cast<int16_t>((static_cast<uint16_t>(th) << 8) | static_cast<uint16_t>(tl));
