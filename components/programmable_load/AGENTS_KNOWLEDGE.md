...
- Charger_14 integration consumes BQ25756 `charge_enable`, `ibat_current` (mA), `vbat_voltage` (mV), `charge_status`, and `status_flags` entities. The core keeps charge enable off outside the charging phase and never permits load current while charging is commanded or observed.
