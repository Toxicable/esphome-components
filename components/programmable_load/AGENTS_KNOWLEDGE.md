...
-- The control loop uses a single `deadband_a` (default 0.010 A). There is no separate near-target/response gating — ramp increments decrease with error magnitude (tiered: fast >5A, medium >2A, then fixed steps down to 0.003A near target).
- Unconfirmed-move tracking computes the gap directly each loop: `unconfirmed_rise = max(0, cmd - i)`, `unconfirmed_fall = max(0, i - cmd)`. When measured current is at or past the command, gap is zero and the loop keeps ramping. This prevents stalls where the accumulator would exhaust its budget before the load responded.
