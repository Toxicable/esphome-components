...
class DRV8243Output : public Component, public output::FloatOutput {
 public:
  ...
  void set_out2_pin(GPIOPin* pin) {
    out2_pin_ = pin;
  }
  void set_flip_polarity(bool v) {
    flip_polarity_ = v;
  }

  void set_out1_output(output::FloatOutput* out) {
    out1_output_ = out;
  }
  void set_out2_output(output::FloatOutput* out) {
    out2_output_ = out;
  }
  void set_out1_component(Component* comp) {
    out1_component_ = comp;
  }
  void set_out2_component(Component* comp) {
    out2_component_ = comp;
  }
  void set_ch1_output(DRV8243ChannelOutput* out) {
    ch1_output_ = out;
  }
  void set_ch2_output(DRV8243ChannelOutput* out) {
    ch2_output_ = out;
  }
  void set_min_level(float v) {
    min_level_ = v;
  }
...
class DRV8243ChannelOutput : public Component, public output::FloatOutput {
 public:
  void set_parent(DRV8243Output* parent) {
    parent_ = parent;
  }
