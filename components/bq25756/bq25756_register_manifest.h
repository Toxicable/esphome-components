...
constexpr RegisterManifestEntry reg(const char *name, RegisterId id, uint8_t width,
                                    uint32_t configuration_mask, uint32_t runtime_mask,
                                    uint32_t status_mask, uint32_t command_mask,
                                    uint32_t reserved_mask) {
  return {name, register_address(id), width, configuration_mask, runtime_mask,
          status_mask, command_mask, reserved_mask};
}

static constexpr std::array<RegisterManifestEntry, REGISTER_COUNT> REGISTER_MANIFEST{{
    reg("charge_voltage_limit", RegisterId::CHARGE_VOLTAGE_LIMIT, 2, 0x001F, 0, 0, 0, 0xFFE0),
    reg("charge_current_limit", RegisterId::CHARGE_CURRENT_LIMIT, 2, 0x07FC, 0, 0, 0, 0xF803),
    reg("input_current_dpm_limit", RegisterId::INPUT_CURRENT_DPM_LIMIT, 2, 0x07FC, 0, 0, 0, 0xF803),
    reg("input_voltage_dpm_limit", RegisterId::INPUT_VOLTAGE_DPM_LIMIT, 2, 0x3FFC, 0, 0, 0, 0xC003),
    reg("reverse_input_current_limit", RegisterId::REVERSE_INPUT_CURRENT_LIMIT, 2, 0x07FC, 0, 0, 0, 0xF803),
    reg("reverse_input_voltage_limit", RegisterId::REVERSE_INPUT_VOLTAGE_LIMIT, 2, 0x3FFC, 0, 0, 0, 0xC003),
    reg("precharge_current_limit", RegisterId::PRECHARGE_CURRENT_LIMIT, 2, 0x03FC, 0, 0, 0, 0xFC03),
    reg("termination_current_limit", RegisterId::TERMINATION_CURRENT_LIMIT, 2, 0x03FC, 0, 0, 0, 0xFC03),
    reg("precharge_termination_control", RegisterId::PRECHARGE_TERMINATION_CONTROL, 1, 0x0F, 0, 0, 0, 0xF0),
    reg("timer_control", RegisterId::TIMER_CONTROL, 1, 0xFF, 0, 0, 0, 0),
    reg("three_stage_charge_control", RegisterId::THREE_STAGE_CHARGE_CONTROL, 1, 0x0F, 0, 0, 0, 0xF0),
    reg("charger_control", RegisterId::CHARGER_CONTROL, 1, 0xD8, 0x07, 0, 0x20, 0),
    reg("pin_control", RegisterId::PIN_CONTROL, 1, 0xFF, 0, 0, 0, 0),
    reg("power_path_control", RegisterId::POWER_PATH_CONTROL, 1, 0x20, 0x41, 0, 0x80, 0x1E),
    reg("mppt_control", RegisterId::MPPT_CONTROL, 1, 0x67, 0, 0, 0x80, 0x18),
    reg("ts_charging_threshold_control", RegisterId::TS_CHARGING_THRESHOLD_CONTROL, 1, 0xFF, 0, 0, 0, 0),
    reg("ts_charging_region_control", RegisterId::TS_CHARGING_REGION_CONTROL, 1, 0x7F, 0, 0, 0, 0x80),
    reg("ts_reverse_threshold_control", RegisterId::TS_REVERSE_THRESHOLD_CONTROL, 1, 0xE0, 0, 0, 0, 0x1F),
    reg("reverse_undervoltage_control", RegisterId::REVERSE_UNDERVOLTAGE_CONTROL, 1, 0x20, 0, 0, 0, 0xDF),
    reg("vac_mpp_detected", RegisterId::VAC_MPP_DETECTED, 2, 0, 0, 0x3FFC, 0, 0xC003),
    reg("charger_status_1", RegisterId::CHARGER_STATUS_1, 1, 0, 0, 0xEF, 0, 0x10),
    reg("charger_status_2", RegisterId::CHARGER_STATUS_2, 1, 0, 0, 0xF3, 0, 0x0C),
    reg("charger_status_3", RegisterId::CHARGER_STATUS_3, 1, 0, 0, 0x3C, 0, 0xC3),
    reg("fault_status", RegisterId::FAULT_STATUS, 1, 0, 0, 0xFE, 0, 0x01),
    reg("charger_flag_1", RegisterId::CHARGER_FLAG_1, 1, 0, 0, 0xFF, 0, 0),
    reg("charger_flag_2", RegisterId::CHARGER_FLAG_2, 1, 0, 0, 0xFF, 0, 0),
    reg("fault_flag", RegisterId::FAULT_FLAG, 1, 0, 0, 0xFF, 0, 0),
    reg("charger_mask_1", RegisterId::CHARGER_MASK_1, 1, 0xEB, 0, 0, 0, 0x14),
    reg("charger_mask_2", RegisterId::CHARGER_MASK_2, 1, 0x9B, 0, 0, 0, 0x64),
    reg("fault_mask", RegisterId::FAULT_MASK, 1, 0xFE, 0, 0, 0, 0x01),
    reg("adc_control", RegisterId::ADC_CONTROL, 1, 0xF8, 0, 0, 0x04, 0x03),
    reg("adc_channel_control", RegisterId::ADC_CHANNEL_CONTROL, 1, 0xF6, 0, 0, 0, 0x09),
    reg("iac_adc", RegisterId::IAC_ADC, 2, 0, 0, 0xFFFF, 0, 0),
    reg("ibat_adc", RegisterId::IBAT_ADC, 2, 0, 0, 0xFFFF, 0, 0),
    reg("vac_adc", RegisterId::VAC_ADC, 2, 0, 0, 0xFFFF, 0, 0),
    reg("vbat_adc", RegisterId::VBAT_ADC, 2, 0, 0, 0xFFFF, 0, 0),
    reg("ts_adc", RegisterId::TS_ADC, 2, 0, 0, 0x03FF, 0, 0xFC00),
    reg("vfb_adc", RegisterId::VFB_ADC, 2, 0, 0, 0x07FF, 0, 0xF800),
    reg("gate_driver_strength_control", RegisterId::GATE_DRIVER_STRENGTH_CONTROL, 1, 0xFF, 0, 0, 0, 0),
    reg("gate_driver_dead_time_control", RegisterId::GATE_DRIVER_DEAD_TIME_CONTROL, 1, 0x0F, 0, 0, 0, 0xF0),
    reg("part_information", RegisterId::PART_INFORMATION, 1, 0, 0, 0x7F, 0, 0x80),
    reg("reverse_battery_discharge_current", RegisterId::REVERSE_BATTERY_DISCHARGE_CURRENT, 1, 0xC2, 0, 0, 0, 0x3D),
}};
