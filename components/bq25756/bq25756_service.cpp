...
bool Bq25756Service::write_u16_le(uint8_t reg, uint16_t value) {
  uint8_t raw[2] = {0, 0};
  component_common::store_le(value, raw);
...
bool Bq25756Service::read_u16_le(uint8_t reg, Reg16Value &value) {
  ...
  value.lsb = raw[0];
  value.msb = raw[1];
  value.raw_le = component_common::load_le<uint16_t>(raw);
  return true;
}
...
bool Bq25756Service::update_register_bits(uint8_t reg, uint8_t mask, uint8_t value_bits) {
  ...
  const uint8_t updated = component_common::replace_masked(current, mask, value_bits);
...
bool Bq25756Service::probe(uint8_t &part_info) {
  ...
  return fields::PartNumber::decode(part_info) == fields::PartNumber::decode(BQ25756_PART_NUM_BITS);
}
...
bool Bq25756Service::set_watchdog_code(uint8_t code) {
  if (!fields::Watchdog::fits(code)) {
    return false;
  }
  return this->update_register_bits(REG15_TIMER_CONTROL, REG15_WATCHDOG_MASK, fields::Watchdog::encode(code));
}
...
bool Bq25756Service::read_control_states(ControlStates &states) {
  ...
  states.charge_enabled = component_common::any_set(reg17, REG17_EN_CHG_MASK);
  states.hiz_mode = component_common::any_set(reg17, REG17_EN_HIZ_MASK);
  states.reverse_mode = component_common::any_set(reg19, REG19_EN_REV_MASK);
  states.watchdog_code = fields::Watchdog::decode(reg15);
  return true;
}

bool Bq25756Service::ensure_adc_enabled(bool include_vfb, uint8_t &reg2b_old, uint8_t &reg2b_new,
                                       uint8_t &reg2c_old, uint8_t &reg2c_new) {
  ...
  // Own the complete conversion-mode field instead of inheriting the POR
  // 13-bit setting or stale averaging state.
  reg2b_new = component_common::replace_masked(reg2b_old, REG2B_ADC_OWNED_MASK, REG2B_ADC_CONTINUOUS_15_BIT);

  reg2c_new = component_common::replace_masked(reg2c_old, REG2C_ADC_CHANNEL_OWNED_MASK, 0x00);
  if (!include_vfb) {
  ...
  return verify[0] == reg2b_new && verify[1] == reg2c_new;
}
...
bool Bq25756Service::read_charge_precheck(ChargePrecheckSnapshot &snapshot) {
  ...
  snapshot.en_chg = component_common::any_set(snapshot.reg17, REG17_EN_CHG_MASK);
  snapshot.en_hiz = component_common::any_set(snapshot.reg17, REG17_EN_HIZ_MASK);
  snapshot.dis_ce_pin = component_common::any_set(snapshot.reg17, REG17_DIS_CE_PIN_MASK);
  snapshot.en_rev = component_common::any_set(snapshot.reg19, REG19_EN_REV_MASK);
  return true;
}
