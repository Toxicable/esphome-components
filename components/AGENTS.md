...
- Reusable chip components should follow `../ARCHITECTURE.md`: host-independent `*_protocol.*`, `*_bus.h`, and `*_service.*` files plus an ESPHome wrapper in `<name>.h/.cpp`.
- Core service class names must be component-specific, such as `HusbService` or `Bq25756Service`, not generic exported names like `Service`.
