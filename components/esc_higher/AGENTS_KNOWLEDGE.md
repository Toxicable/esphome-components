...
- Exposed command buttons map to opcodes:
  ...
  - `estop` -> `0x05`
  - Bring-up controls:
    - `bringup_test_select` chooses `101` (`full_spin_sequence`, default) or `102` (`bridge_static_vector_test`)
    - `run_bringup_test` -> `0x09` (`param0=<selected test_id>`, `param1=bringup_test_duration_ms` for `101` or `50` for `102`, `param2=bringup_test_options`)
...
- Host bring-up commands now carry an explicit test ID. Supported IDs are `101` (`full_spin_sequence`, default) and `102` (`bridge_static_vector_test`); `BRINGUP.test_id` remains the report field.
