...
- `components/mcf8316d_manual` now supports an optional `apply_startup_tune` button that forces `speed=0%`, `direction=cw`, `brake=off`, writes a RAM-only startup profile (`FAULT_CONFIG1`, `FAULT_CONFIG2`, `MOTOR_STARTUP1`, `MOTOR_STARTUP2`, `CLOSED_LOOP4`) including `MTR_STARTUP=IPD`, `MAX_SPEED=0x2710`, `HW_LOCK_ILIMIT=8A`, `HW_LOCK_ILIMIT_DEG=7us`, then pulses `CLR_FLT`.
