...
- `components/mcf8316d_manual` now supports an optional `apply_startup_tune` button that writes a RAM-only startup profile (`FAULT_CONFIG1`, `MOTOR_STARTUP1`, `MOTOR_STARTUP2`) for lock-limit bring-up and then pulses `CLR_FLT`.
