...
- `components/mcf8316d_manual` now also supports an optional `apply_hw_lock_report_only` debug button that sets `FAULT_CONFIG2.HW_LOCK_ILIMIT_MODE=report_only` (6) for temporary no-load diagnostics, then pulses `CLR_FLT`.
- `components/mcf8316d_manual` `apply_startup_tune` now explicitly restores `FAULT_CONFIG2.HW_LOCK_ILIMIT_MODE=retry_hiz` (3), so running startup tune exits report-only debug mode.
